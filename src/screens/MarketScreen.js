...
export function MarketScreen({
      ...
      <>
        ...
        <SectionCard title="Handel" subtitle="Kupuj tanio, sprzedawaj drozej. Ceny odswiezaja sie automatycznie.">
          ...
          {products.map((product) => {
            const locked = game.player.respect < product.unlockRespect;
            const buyPrice = game.market[product.id];
            const sellPrice = Math.floor(game.market[product.id] * 0.85);
            const snapshot = marketState?.[product.id];
              ...
              <View key={product.id} style={[styles.marketRow, locked && styles.listCardLocked]}>
                <View style={styles.entityHead}>
                  ...
                  <View style={styles.marketInfo}>
                    ...
                    <Text style={styles.marketMeta}>Wymaga {product.unlockRespect} szacunu | Na stanie: {game.inventory[product.id]}</Text>
            ...
            <View key={drug.id} style={styles.listCard}>
              <View style={styles.listCardHeader}>
                <View style={styles.entityHead}>
                  ...
                  <View style={styles.flexOne}>
                    ...
                    <Text style={styles.listCardMeta}>
                      Przy Tobie: {game.drugInventory[drug.id]} | U dilera: {game.dealerInventory?.[drug.id] || 0} | Ulica: {formatMoney(drug.streetPrice)}
                    </Text>
                  </View>
                </View>
              ...
              <View style={styles.oddsRow}>
                <View style={styles.oddsBlock}>
                  ...
                  <Text style={styles.oddsValue}>{game.dealerInventory?.[drug.id] || 0}</Text>
                </View>
                <View style={styles.oddsBlock}>
              ...
              <View style={styles.marketButtons}>
                <Pressable
                  ...
                  style={[styles.marketButton, (game.player.respect < drug.unlockRespect || (game.dealerInventory?.[drug.id] || 0) <= 0) && styles.tileDisabled]}
                >
                  <Text style={styles.marketButtonText}>Kup od dilera</Text>
                </Pressable>
                <Pressable onPress={() => actions.sellDrugToDealer(drug)} style={[styles.marketButton, game.drugInventory[drug.id] <= 0 && styles.tileDisabled]}>
                  <Text style={styles.marketButtonText}>Sprzedaj dilerowi</Text>
                </Pressable>
                <Pressable onPress={() => actions.consumeDrug(drug)} style={[styles.marketButton, game.drugInventory[drug.id] <= 0 && styles.tileDisabled]}>
  ...
  return (
    <SectionCard title="Aktywne boosty" subtitle="Tu widzisz wszystko, co teraz podbija staty.">
      ...
      {!game.activeBoosts.length ? <Text style={styles.emptyText}>Brak aktywnych efektow. Wszystko na sucho.</Text> : null}
      {game.activeBoosts.map((boost) => (
        <View key={boost.id} style={styles.listCard}>
          <View style={styles.inlineRow}>
            ...
            <Tag text={formatDuration(boost.expiresAt - Date.now())} />
          </View>
        </View>
      ))}
    </SectionCard>
