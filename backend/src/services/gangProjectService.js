...
export function joinGangForPlayer(player, invite, now = Date.now(), gangSnapshot = null) {
  ...
  player.gang = ensureGangWeeklyGoal(
    createGangState({
      ...
      role: "Czlonek",
      name: gangName,
      identity: gangSnapshot?.identity,
      contactNetwork: gangSnapshot?.contactNetwork,
      cityResponse: gangSnapshot?.cityResponse,
      directorResponse: gangSnapshot?.directorResponse,
