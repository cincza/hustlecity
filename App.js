...
const AVATAR_OPTIONS = [
  { id: "ghost", name: "Ghost", sigil: "GX", colors: ["#4f4f52", "#161618"] },
  { id: "razor", name: "Razor", sigil: "RZ", colors: ["#6a2a1d", "#1b0d0a"] },
  { id: "saint", name: "Saint", sigil: "ST", colors: ["#4b3914", "#171109"] },
  { id: "vandal", name: "Vandal", sigil: "VN", colors: ["#22463f", "#0d1816"] },
  { id: "venom", name: "Venom", sigil: "VN", colors: ["#243055", "#0a0e19"] },
  { id: "boss", name: "Boss", sigil: "$$", colors: ["#5a4b1f", "#171308"] },
];
...
const CHARACTER_ARCHETYPES = [
  {
    ...
    starter: { respect: 4, attack: 7, defense: 7, dexterity: 8, charisma: 12, energy: 13, hp: 98, cash: 4800, bank: 2000 },
  },
  {
    id: "fighter",
    name: "Fighter",
    ...
    starter: { respect: 3, attack: 12, defense: 10, dexterity: 7, charisma: 5, energy: 14, hp: 108, cash: 3600, bank: 1200 },
  },
  {
    id: "runner",
    name: "Runner",
    ...
    starter: { respect: 3, attack: 8, defense: 7, dexterity: 13, charisma: 6, energy: 15, hp: 100, cash: 3900, bank: 1400 },
  },
  {
    id: "broker",
    name: "Broker",
    ...
    starter: { respect: 5, attack: 6, defense: 6, dexterity: 8, charisma: 10, energy: 13, hp: 96, cash: 6200, bank: 3500 },
  },
];
...
const TAB_DEFINITIONS = [
  { id: "city", label: "Miasto", sections: [{ id: "dashboard", label: "Miasto", title: "Miasto" }, { id: "tasks", label: "Zadania", title: "Zadania" }, { id: "bank", label: "Bank", title: "Bank" }, { id: "gym", label: "Silownia", title: "Silownia" }, { id: "restaurant", label: "Restauracja", title: "Restauracja" }, { id: "hospital", label: "Szpital", title: "Szpital" }, { id: "casino", label: "Kasyno", title: "Kasyno" }] },
  { id: "heists", label: "Napady", sections: [{ id: "solo", label: "Napady", title: "Napady" }, { id: "gang", label: "Napady gangu", title: "Napady gangu" }, { id: "fightclub", label: "Fightclub", title: "Fightclub" }, { id: "prison", label: "Wiezienie", title: "Wiezienie" }] },
  { id: "empire", label: "Imperium", sections: [{ id: "businesses", label: "Biznesy", title: "Biznesy" }, { id: "factories", label: "Fabryki", title: "Fabryki" }, { id: "suppliers", label: "Hurtownie", title: "Hurtownie" }, { id: "club", label: "Klub", title: "Klub" }] },
  { id: "market", label: "Rynek", sections: [{ id: "street", label: "Handel", title: "Handel" }, { id: "drugs", label: "Narkotyki", title: "Narkotyki" }, { id: "boosts", label: "Boosty", title: "Boosty" }] },
  { id: "gang", label: "Gang", sections: [{ id: "overview", label: "Szukaj gangu", title: "Gang" }, { id: "members", label: "Czlonkowie", title: "Czlonkowie" }, { id: "chat", label: "Chat gangu", title: "Chat gangu" }, { id: "ops", label: "Operacje", title: "Operacje" }] },
  { id: "online", label: "Online", sections: [{ id: "players", label: "Gracze", title: "Gracze" }, { id: "friends", label: "Znajomi", title: "Znajomi" }, { id: "messages", label: "Wiadomosci", title: "Wiadomosci" }, { id: "rankings", label: "Rankingi", title: "Rankingi" }] },
  { id: "premium", label: "Premium", sections: [{ id: "shop", label: "Mikroplatnosci", title: "Mikroplatnosci" }, { id: "services", label: "Uslugi", title: "Uslugi premium" }, { id: "referrals", label: "Polecenia", title: "Program polecen" }] },
  { id: "profile", label: "Profil", sections: [{ id: "summary", label: "Profil", title: "Profil" }, { id: "progress", label: "Szacun", title: "Szacun" }, { id: "protection", label: "Ochrona", title: "Ochrona" }, { id: "log", label: "Log wydarzen", title: "Log wydarzen" }] },
];
...
const INITIAL = {
  ...
  online: {
    roster: WORLD_PLAYERS.map((player) => ({ ...player })),
    selectedPlayerId: null,
    selectedGangId: null,
    friends: [],
    messages: [
      { id: "msg-1", from: "System", subject: "Miasto patrzy", preview: "Klikaj profile graczy, sprawdzaj bounty i pilnuj kto rosnie za szybko.", time: "14:22" },
    ],
  },
  referrals: {
...
const formatMoney = (value) => `$${Math.floor(value).toLocaleString("en-US")}`;
...
export default function App() {
  ...
  const mergeServerUser = (serverUser, marketPayload) => {
    setGame((prev) => ({
      ...
      player: {
        ...prev.player,
        name: serverUser.profile.name,
        avatarId: serverUser.profile.avatarId || prev.player.avatarId,
        rank: getRankTitle(serverUser.profile.respect),
        cash: serverUser.profile.cash,
        energy: serverUser.profile.energy,
        maxEnergy: serverUser.profile.maxEnergy || prev.player.maxEnergy,
        hp: clamp(serverUser.profile.hp, 0, prev.player.maxHp),
        respect: serverUser.profile.respect,
        attack: serverUser.profile.attack,
        defense: serverUser.profile.defense,
        dexterity: serverUser.profile.dexterity ?? prev.player.dexterity,
        charisma: serverUser.profile.charisma,
        heat: serverUser.profile.heat,
        bank: serverUser.profile.bank,
      },
      stats: { ...prev.stats, ...serverUser.stats },
      inventory: serverUser.inventory || prev.inventory,
      log: serverUser.log || prev.log,
  ...
  const refreshCasinoState = async (token = sessionToken) => {
    ...
    setCasinoState((prev) => ({ ...prev, backendMeta: meta }));
  };
  ...
  const hydrateAuthenticatedSession = async (token) => {
    ...
    setCasinoState((prev) => ({ ...prev, backendMeta: casinoMeta }));
  };
  ...
  const handleAuthLogin = async ({ login, password }) => {
    ...
    try {
      ...
      await saveStoredAuthToken(result.token);
  ...
  // TODO: TO_MIGRATE_TO_SERVER - blackjack must be resolved server-side with authoritative deck,
  // hidden dealer state, bet validation and audit logs before multiplayer launch.
  ...
  const renderOnlinePlayers = () => (
    <>
      <SceneArtwork
        eyebrow="Gracze"
        title="Kogo spotkasz na miescie"
        lines={["Lista ma byc czytelna od razu: kto ma respekt, kto ma hajs i z kim warto sie liczyc.", "To jest baza pod docelowe ataki, znajomych i prywatne wiadomosci online."]}
        accent={["#342418", "#140f0c", "#050505"]}
        source={SCENE_BACKGROUNDS.gang}
      />
      {selectedWorldPlayer ? (
        <SectionCard title="Profil gracza" subtitle="Tak ma to dzialac: klikasz kogos z listy i wpadasz w jego karte bez pelnych sekretow konta.">
          <View style={styles.playerProfilePanel}>
            <View style={styles.playerProfileMain}>
      ...
      <SectionCard title="Gracze" subtitle="Kliknij gracza, a otworzy sie jego karta z podstawowymi statami i akcjami.">
        ...
        <View style={styles.pageBackdrop}>
          ...
          <View style={styles.gameShell}>
              ...
              <View style={[styles.topStripContent, isPhone && styles.topStripContentPhone]}>
                <View style={[styles.topStripProfile, isPhone && styles.topStripProfilePhone]}>
                  <LinearGradient colors={activeAvatar.colors} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.profileShot}>
                    <Text style={styles.profileShotSigil}>{activeAvatar.sigil}</Text>
            ...
            <View style={styles.mastheadPanel}>
              <ImageBackground source={BRAND_MASTHEAD} style={styles.mastheadImage} imageStyle={styles.mastheadImageInner}>
                <LinearGradient colors={["rgba(0,0,0,0.05)", "rgba(0,0,0,0.18)", "rgba(0,0,0,0.72)"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.mastheadOverlay}>
                  <View style={[styles.mastheadStatus, isPhone && styles.mastheadStatusPhone]}>
                    <Text style={styles.mastheadStatusTitle}>Miasto zyje</Text>
                    <Text style={styles.mastheadStatusText}>
                      {inJail(game.player)
                        ? `Siedzisz za krata jeszcze ${formatDuration(jailRemaining)}.`
                        : "Pompuj staty, rozkrec klub, fabryki i wejdz na poziom, gdzie cale miasto zna Twoja ksywe."}
                    </Text>
                  </View>
                </LinearGradient>
              </ImageBackground>
            ...
            <View style={styles.tabBarClassic}>
            ...
            <View style={[styles.mainBoard, isCompact && styles.mainBoardCompact]}>
              <View style={[styles.leftRail, isCompact && styles.railCompact, isPhone && styles.railPhone]}>
              ...
              <View style={[styles.rightRail, isCompact && styles.railCompact, isPhone && styles.railPhone]}>
...
const styles = StyleSheet.create({
  ...
  pageScroll: { paddingVertical: 18, paddingBottom: 42, backgroundColor: "#000000" },
  pageBackdrop: { position: "relative", paddingHorizontal: 10 },
  ...
  mastheadOverlay: { minHeight: 210, justifyContent: "flex-end" },
  mastheadStatus: {
    alignSelf: "flex-end",
    width: "36%",
    minWidth: 280,
    ...
    borderWidth: 1,
    borderColor: "rgba(255,201,111,0.28)",
  },
  mastheadStatusPhone: { width: "100%", minWidth: 0, alignSelf: "stretch", margin: 0, marginTop: 120 },
  mastheadStatusTitle: { color: "#f4d37e", fontSize: 16, fontWeight: "900", marginBottom: 6, textTransform: "uppercase", letterSpacing: 1 },
  mastheadStatusText: { color: "#f0e6d8", fontSize: 12, lineHeight: 18 },
  ...
  tabBarClassic: { flexDirection: "row", borderTopWidth: 1, borderBottomWidth: 1, borderColor: "#3c3c3c", backgroundColor: "#0b0b0b" },
  tabButtonClassic: { flex: 1, paddingVertical: 10, borderRightWidth: 1, borderColor: "#2f2f2f", backgroundColor: "#0d0d0d", alignItems: "center" },
  tabButtonClassicActive: { backgroundColor: "#181818" },
  tabButtonClassicText: { color: "#b7b7b7", fontSize: 16, fontWeight: "700" },
  ...
  contentHeaderLabel: { color: "#f0c24d", fontSize: 22, fontWeight: "800" },
  contentHeaderSub: { color: "#666666", fontSize: 11, textTransform: "uppercase" },
  sectionCard: { backgroundColor: "rgba(12,12,13,0.95)", borderRadius: 2, padding: 14, borderWidth: 1, borderColor: "#2a2a2d", marginBottom: 8 },
  ...
  actionTile: { width: "48%", minHeight: 110, padding: 14, borderRadius: 2, backgroundColor: "#141414", borderWidth: 1, borderColor: "#303030", justifyContent: "space-between" },
  ...
  listCard: { padding: 12, borderRadius: 2, backgroundColor: "#111214", borderWidth: 1, borderColor: "#2b2b31", marginBottom: 10 },
  districtCard: { padding: 10, borderRadius: 2, backgroundColor: "#17181c", borderWidth: 1, borderColor: "#2f3138", marginTop: 10 },
  ...
  sceneArtwork: { minHeight: 168, marginBottom: 12, borderWidth: 1, borderColor: "#34271f", overflow: "hidden", justifyContent: "flex-end" },
  ...
  avatarHero: { minHeight: 120, borderWidth: 1, borderColor: "#373737", padding: 14, justifyContent: "space-between" },
  ...
  avatarChoiceArt: { width: "100%", height: 66, alignItems: "center", justifyContent: "center" },
